...
def ns_loadings_vec(tau_arr: np.ndarray, lam: float) -> Tuple[np.ndarray, np.ndarray]:
    """Vectorised Nelson-Siegel loadings over a maturity array."""
...
def fit_ns_betas_ols_row(
    ...
    """
    Single-row OLS fit of NS β₀, β₁, β₂ for a given λ.
    Returns dict with keys beta0/beta1/beta2/rmse, or None if too few points.
    Mirrors the notebook's fit_betas_ols() exactly.
    """
    tau_arr = np.asarray(tau_arr, dtype=float)
    ...
    return {"beta0": float(coef[0]), "beta1": float(coef[1]),
            "beta2": float(coef[2]), "rmse": rmse}
...
SUBMISSION_PATH = Path(
    os.getenv("CODE4_SUBMISSION_PATH",
              str(PROJECT_ROOT / "outputs" / "Problem_1_yield_curve_predict.xlsx"))
)
...
# Nelson-Siegel λ grid for a short ON–2Y curve.
# Lower bound 0.2: below this the curvature hump is at τ < 0.2Y ≈ 10 weeks,
#   placing it below even the shortest meaningful tenor → β₂ unidentified.
# Upper bound 1.5: at λ=1.5 the hump is at τ=1.5Y (within range, still OK).
#   Above 1.5 the curvature loading is nearly flat across 0–2Y → again degenerate.
# 40-point grid is sufficient (finer grids add cost with no benefit given OLS noise).
# NS_HORIZON: the CV evaluates at the actual forecast horizon (6 months = TEST_SIZE)
#   rather than h=1 (notebook default), matching the real task.
NS_LAM_GRID = np.linspace(0.20, 1.50, 40)
NS_INITIAL_TRAIN = 24  # minimum obs before first OOS evaluation
NS_CV_HORIZON = 6  # evaluate lambda by h=6 OOS RMSE, not h=1
...
def find_best_arima_orders(df_b_train, df_b_val, beta_cols=BETA_COLS):
    ...
    for b in beta_cols:
        ...
        if len(y_tr) < 10 or len(y_va) == 0:
            best[b] = (2, choose_d_by_adf(y_tr), 0);
            continue
...
# ══════════════════════════════════════════════════════════════════════════════
# §7e  SVENSSON ARIMAX FORECASTER  (IV-augmented variant of §7d)
# ══════════════════════════════════════════════════════════════════════════════

class SvenssonARIMAXForecaster(SvenssonARIMAForecaster):
    """
    Svensson betas forecast via ARIMAX(macro_pca + predicted iv_pc1).

    Strategy:
    ─────────
    1. Pre-fit AR(1) on iv_pc1 to produce out-of-sample iv_pc1 forecasts.
    2. For each SV beta, fit ARIMAX using [macro_pca | iv_pc1] as exogenous.
    3. At forecast time, supply projected macro_pca + AR(1)-predicted iv_pc1.
    """
    ...
    def __init__(
        ...
        super().__init__(yc_cols, maturity_map, lam1_grid, lam2_grid, best_orders)
        self._arimax: Dict = {}  # beta → fitted ARIMAX result
        self._exog_tr: Dict = {}  # beta → exog DataFrame used in fit
        self._ecols_tr: Dict = {}  # beta → list of exog column names
        self._iv_ar1: Optional[object] = None  # AR(1) for iv_pc1 projection
        self._iv_last: float = 0.0

    # ── Phase 1 (override): λ* + ARIMAX orders from train→val ─────────────────

    def fit_x(
            self,
            df_yc_train: pd.DataFrame,
            df_yc_val: pd.DataFrame,
            df_macro_pca_train: pd.DataFrame,
            df_iv_train: pd.DataFrame,
            verbose: bool = True,
    ) -> "SvenssonARIMAXForecaster":
        """Phase 1 for ARIMAX variant — same lambda search, ARIMAX order grid."""
        # 1. Lambda grid on TRAIN
        self.lam1_opt, self.lam2_opt, in_rmse = grid_search_sv_lambdas(
            df_yc_train, self.yc_cols, self.maturity_map,
            self.lam1_grid, self.lam2_grid,
        )
        if verbose:
            print(f"  [SV-X] λ₁*={self.lam1_opt:.2f}  λ₂*={self.lam2_opt:.2f}"
                  f"  train-RMSE={in_rmse:.5f}")

        # 2. OLS betas on TRAIN
        self._betas = fit_sv_betas_ols(
            df_yc_train, self.yc_cols, self.maturity_map,
            self.lam1_opt, self.lam2_opt,
        )

        # 3. ARIMA order search (reuse parent helper — pure AR order for each beta)
        if df_yc_val is not None:
            sv_betas_val = fit_sv_betas_ols(
                df_yc_val, self.yc_cols, self.maturity_map,
                self.lam1_opt, self.lam2_opt,
            ...
            if verbose:
                print("  [SV-X] ARIMA order grid-search …")
            for b in SV_BETA_COLS:
                y_tr = self._betas[b].dropna()
                y_va = sv_betas_val[b].dropna()
                if len(y_tr) < 10 or len(y_va) == 0:
        ...
        # 4. Fit ARIMAX on TRAIN betas
        self._fit_arimax_on_current_betas(df_macro_pca_train, df_iv_train)
        return self

    # ── Phase 2: refit ARIMAX on trainval ─────────────────────────────────────

    def refit_arimax_only(
            self,
            df_yc_new: pd.DataFrame,
            df_macro_pca: pd.DataFrame,
            df_iv: pd.DataFrame,
    ) -> "SvenssonARIMAXForecaster":
        """[F3-analog] Refit OLS betas + ARIMAX on larger dataset, keep λ* & orders."""
        self._betas = fit_sv_betas_ols(
            df_yc_new, self.yc_cols, self.maturity_map,
            self.lam1_opt, self.lam2_opt,
        )
        self._fit_arimax_on_current_betas(df_macro_pca, df_iv)
        return self

    # ── Internal: build AR(1) for iv_pc1 + fit ARIMAX on betas ───────────────

    def _fit_arimax_on_current_betas(
            self,
            df_macro_pca: pd.DataFrame,
            df_iv: pd.DataFrame,
    ) -> None:
        # Pre-fit AR(1) model for iv_pc1 projection
        if "iv_pc1" in df_iv.columns:
            iv_s = df_iv["iv_pc1"].dropna()
            self._iv_last = float(iv_s.iloc[-1]) if len(iv_s) else 0.0
            try:
                d_iv = choose_d_by_adf(iv_s)
                self._iv_ar1 = ARIMA(iv_s, order=(1, d_iv, 0)).fit()
            except Exception:
                self._iv_ar1 = None
        else:
            self._iv_last = 0.0
        ...
        # Build exog: align macro_pca + iv_pc1
        exog_cols = list(df_macro_pca.columns)
        df_exog = df_macro_pca.copy()
        if "iv_pc1" in df_iv.columns:
            df_exog = df_exog.join(df_iv[["iv_pc1"]], how="left").ffill().bfill()
            exog_cols.append("iv_pc1")

        for b in SV_BETA_COLS:
            y = self._betas[b].dropna()
            common = y.index.intersection(df_exog.index)
            if len(common) < 10:
                self._arimax[b] = None
                self._trains[b] = y
                continue
            y1 = y.loc[common].astype(float)
            X1 = df_exog.loc[common, exog_cols].astype(float).ffill().bfill().fillna(0.0)
            # Drop zero-variance columns
            X1 = X1.loc[:, X1.std(ddof=0) > 1e-12]
            if X1.shape[1] == 0:
                X1_fit = None
                ecols = []
            else:
                X1_fit = X1
                ecols = list(X1.columns)
            order = self.best_orders.get(b, (1, choose_d_by_adf(y1), 0))
            try:
                res = (ARIMA(y1, order=order).fit() if X1_fit is None
                       else ARIMA(y1, exog=X1_fit, order=order).fit())
                self._arimax[b] = res
                self._trains[b] = y1
                self._exog_tr[b] = X1_fit
                self._ecols_tr[b] = ecols
            except Exception:
                self._arimax[b] = None
                self._trains[b] = y
    ...
    def forecast_x(
            ...
            df_macro_pca_fut: pd.DataFrame,
    ) -> pd.DataFrame:
        """
        Forecast SV yield curve using ARIMAX(macro_pca + ar1(iv_pc1)).
        iv_pc1 future values are generated from the pre-fitted AR(1).
        """
        # Project iv_pc1 forward
        if self._iv_ar1 is not None:
            try:
                iv_fut_vals = np.asarray(self._iv_ar1.forecast(steps))
            except Exception:
                iv_fut_vals = np.full(steps, self._iv_last)
        else:
            iv_fut_vals = np.full(steps, self._iv_last)

        iv_fut_s = pd.Series(iv_fut_vals, index=fi)

        fc_b = pd.DataFrame(index=fi, columns=SV_BETA_COLS, dtype=float)
        for b in SV_BETA_COLS:
            m = self._arimax.get(b)
            tr = self._trains.get(b, pd.Series(dtype=float))
            lv = float(tr.iloc[-1]) if len(tr) else np.nan
            if m is None:
                fc_b[b] = lv
                continue
            try:
                ecols = self._ecols_tr.get(b, [])
                if not ecols:
                    fc = pd.Series(np.asarray(m.forecast(steps)), index=fi)
                else:
                    # Build future exog aligned to ecols
                    Xf = df_macro_pca_fut.reindex(fi).copy()
                    if "iv_pc1" in ecols:
                        Xf["iv_pc1"] = iv_fut_s.values
                    Xf = Xf.reindex(columns=ecols).ffill().bfill().fillna(0.0)
                    fc = pd.Series(np.asarray(m.forecast(steps, exog=Xf)), index=fi)
                fc_b[b] = clip_forecast(fc, tr)
            except Exception:
                fc_b[b] = lv
...
# ══════════════════════════════════════════════════════════════════════════════
# §7f  NELSON-SIEGEL AR(1) FORECASTER  (notebook model)
# ══════════════════════════════════════════════════════════════════════════════

# ══════════════════════════════════════════════════════════════════════════════
# §7f  NELSON-SIEGEL AR(1) FORECASTER  (corrected notebook model)
# ══════════════════════════════════════════════════════════════════════════════

class NelsonSiegelAR1Forecaster:
    """
    Nelson-Siegel model with AR(1) beta dynamics.

    Three corrections vs the notebook (nelson.ipynb):

    C1 — METRIC: λ CV uses the tournament's weighted RMSE (ON=0.4, rest=0.6)
         instead of unweighted average-tenor RMSE.  This aligns λ selection
         with the actual scoring criterion.

    C2 — HORIZON: CV evaluates h = NS_CV_HORIZON (=6) step-ahead OOS RMSE
         rather than h=1 (notebook default).  AR(1) iterated 6 times has
         materially different λ sensitivity than 1-step-ahead.
         Implementation: for each expanding window ending at t, fit AR(1) on
         betas[0..t], iterate it NS_CV_HORIZON times, reconstruct the curve,
         compare against Y[t + NS_CV_HORIZON].

    C3 — GRID: restricted to λ ∈ [0.20, 1.50] (40 points).  Below 0.20 the
         curvature hump lies below the shortest meaningful tenor and β₂ is
         unidentified.  Above 1.50 the curvature loading is essentially flat
         across ON–2Y and β₂ again degenerates.  This avoids numerical noise
         from near-singular OLS (same principle as F1 for Svensson).

    Usage (two-phase, consistent with SvenssonARIMAForecaster):
        p1.fit(df_yc_train)        # C1–C3 λ CV on train only
        p2.lam_opt = p1.lam_opt
        p2.refit(df_yc_trainval)   # re-OLS betas on larger window, same λ*
        yc = p2.forecast(6, fi)    # iterated AR(1) × 6
    """
    ...
    def __init__(
            ...
            yc_cols: List[str] = YIELD_COLS,
            maturity_map: Dict[str, float] = MATURITY_MAP,
            lam_grid: np.ndarray = NS_LAM_GRID,
            initial_train: int = NS_INITIAL_TRAIN,
            cv_horizon: int = NS_CV_HORIZON,
            min_points: int = 4,
    ):
        self.yc_cols = yc_cols
        self.maturity_map = maturity_map
        ...
        self.lam_opt: float = NS_LAMBDA
        self._betas: pd.DataFrame = pd.DataFrame()
        self._trains: Dict[str, np.ndarray] = {}

    # ── Phase 1: λ* via expanding-window multi-horizon weighted CV ────────────
    ...
    def fit(
        ...
        """
        Select λ* by expanding-window CV:
          for each t from initial_train to T-cv_horizon-1:
            fit AR(1) on betas[0..t], iterate cv_horizon steps,
            compute weighted RMSE vs Y[t+cv_horizon].
          pick λ = argmin mean weighted OOS RMSE across all windows.
        """
        cols = [c for c in self.yc_cols if c in df_yc_train.columns]
        tau_arr = np.array([self.maturity_map[c] for c in cols], dtype=float)
        Y = df_yc_train[cols].values.astype(float)
        T = len(Y)
        h = self.cv_horizon

        # Weights matching compute_weighted_rmse_curve()
        w_vec = self._tenor_weights(cols)

        best_lam, best_oos = self.lam_opt, np.inf

        for lam in self.lam_grid:
            betas = self._fit_all_betas(Y, tau_arr, lam)  # (T, 3)
            if betas is None:
                continue

            oos_scores = []
            for t in range(self.initial_train, T - h):
                b0_fc, b1_fc, b2_fc = self._ar1_iterated(betas[:t], h)
                if any(np.isnan(v) for v in [b0_fc, b1_fc, b2_fc]):
                    continue
                y_true = Y[t + h]
                ...
                if mask.sum() < self.min_points:
                    continue
                l1, l2 = ns_loadings_vec(tau_arr[mask], lam)
                y_pred = b0_fc + b1_fc * l1 + b2_fc * l2
                # weighted squared error
                wse = float(np.sum(w_vec[mask] * (y_true[mask] - y_pred) ** 2))
                oos_scores.append(wse)

            if not oos_scores:
                continue
            mean_oos = float(np.sqrt(np.nanmean(oos_scores)))
            if mean_oos < best_oos:
                best_oos, best_lam = mean_oos, lam

        self.lam_opt = best_lam
        if verbose:
            print(f"  [NS-AR1] λ*={self.lam_opt:.4f}  "
                  f"mean_weighted_oos_rmse(h={h})={best_oos:.5f}")

        self._store_betas(df_yc_train)
        return self

    # ── Phase 2: refit betas on larger window, same λ* ────────────────────────

    def refit(self, df_yc_new: pd.DataFrame) -> "NelsonSiegelAR1Forecaster":
        """Re-OLS betas on df_yc_new using already-selected λ*."""
        self._store_betas(df_yc_new)
        return self

    # ── Forecast: iterated AR(1) for `steps` periods ─────────────────────────

    def forecast(self, steps: int, fi: pd.DatetimeIndex) -> pd.DataFrame:
        """
        Iterated AR(1) forecast of NS betas → reconstruct YC.
        Each step appends the predicted value so AR(1) auto-regresses on it.
        """
        fc_b = pd.DataFrame(index=fi, columns=BETA_COLS, dtype=float)
        betas_arr = np.column_stack([
            self._trains.get(b, np.array([])) for b in BETA_COLS
        ]) if all(b in self._trains for b in BETA_COLS) else None

        for i, b in enumerate(BETA_COLS):
            series = self._trains.get(b, np.array([]))
            preds, buf = [], series.copy()
            for _ in range(steps):
                v = self._ar1_one_step(buf)
                if np.isnan(v):
                    v = float(buf[-1]) if len(buf) else 0.0
                preds.append(v)
                buf = np.append(buf, v)
            fc_b[b] = preds
        ...
        return reconstruct_yc_from_ns_betas(
            fc_b,
            {k: self.maturity_map[k] for k in self.yc_cols
             if k in self.maturity_map},
            lam=self.lam_opt,
        )

    # ── Internal helpers ───────────────────────────────────────────────────────

    def _tenor_weights(self, cols: List[str]) -> np.ndarray:
        """Per-tenor weight vector matching compute_weighted_rmse_curve()."""
        w = np.full(len(cols), 0.6 / max(len(cols) - 1, 1))
        if "ON" in cols:
            w[cols.index("ON")] = 0.4
        return w

    def _fit_all_betas(
            self, Y: np.ndarray, tau_arr: np.ndarray, lam: float
    ) -> Optional[np.ndarray]:
        """Fit NS betas for every row. Returns (T, 3) or None."""
        rows = []
        for y in Y:
            fit = fit_ns_betas_ols_row(tau_arr, y, lam, self.min_points)
            rows.append([fit["beta0"], fit["beta1"], fit["beta2"]]
                        if fit else [np.nan, np.nan, np.nan])
        arr = np.array(rows, dtype=float)
        return None if np.all(np.isnan(arr)) else arr

    @staticmethod
    def _ar1_one_step(series: np.ndarray) -> float:
        """OLS AR(1) one-step-ahead. Returns NaN if fewer than 5 obs."""
        ...
        return float(coef[0] + coef[1] * s[-1])

    def _ar1_iterated(
            self, betas: np.ndarray, steps: int
    ) -> Tuple[float, float, float]:
        """
        Iterate AR(1) `steps` times on each beta column.
        Returns the final predicted (b0, b1, b2).
        """
        vals = []
        for col in range(3):
            buf = betas[:, col].copy()
    ...
    def _store_betas(self, df_yc: pd.DataFrame) -> None:
        cols = [c for c in self.yc_cols if c in df_yc.columns]
        tau_arr = np.array([self.maturity_map[c] for c in cols], dtype=float)
        Y = df_yc[cols].values.astype(float)
        arr = self._fit_all_betas(Y, tau_arr, self.lam_opt)
        if arr is None:
            arr = np.full((len(Y), 3), np.nan)
        self._betas = pd.DataFrame(arr, index=df_yc.index,
                                   columns=["beta0", "beta1", "beta2"])
        for b in ["beta0", "beta1", "beta2"]:
            self._trains[b] = self._betas[b].dropna().values.copy()


class NelsonSiegelAR1XForecaster(NelsonSiegelAR1Forecaster):
    """
    IV-augmented Nelson-Siegel: same λ CV as base class, but replaces
    iterated AR(1) beta forecasts with ARIMAX(macro_pca + iv_pc1).
    iv_pc1 future values are produced by a pre-fitted AR(1) on iv_pc1.

    Note: λ selection CV still uses pure AR(1) (not ARIMAX) for speed
    and to avoid the exog-future leakage problem inside CV loops.
    λ choice is driven by the curve's cross-sectional structure and
    multi-step beta dynamics — both of which are independent of exog.
    """
    ...
    def fit_x(
            ...
            df_iv_train: pd.DataFrame,
            verbose: bool = True,
    ) -> "NelsonSiegelAR1XForecaster":
        """Phase 1: λ* via C1-C3 CV on train, then ARIMAX on betas."""
        self.fit(df_yc_train, verbose=verbose)
        self._fit_arimax(df_macro_pca_train, df_iv_train)
        return self
    ...
    def refit_x(
        ...
        """Phase 2: re-OLS betas on larger window, same λ*, refit ARIMAX."""
        self._store_betas(df_yc_new)
        self._fit_arimax(df_macro_pca, df_iv)
        return self
    ...
    def _fit_arimax(
        ...
        # AR(1) for iv_pc1 projection at forecast time
        if "iv_pc1" in df_iv.columns:
            iv_s = df_iv["iv_pc1"].dropna()
            self._iv_last = float(iv_s.iloc[-1]) if len(iv_s) else 0.0
            try:
                self._iv_ar1_model = ARIMA(
                    iv_s, order=(1, choose_d_by_adf(iv_s), 0)
                ).fit()
            except Exception:
                self._iv_ar1_model = None
        else:
            self._iv_last, self._iv_ar1_model = 0.0, None

        df_exog = df_macro_pca.copy()
        if "iv_pc1" in df_iv.columns:
            df_exog = df_exog.join(df_iv[["iv_pc1"]], how="left").ffill().bfill()
        exog_cols = list(df_exog.columns)

        for b in BETA_COLS:
            y = self._betas[b].dropna()
            common = y.index.intersection(df_exog.index)
            if len(common) < 10:
                self._arimax[b], self._trains[b] = None, y.values
                continue
            y1 = y.loc[common].astype(float)
            X1 = (df_exog.loc[common, exog_cols]
                  .astype(float).ffill().bfill().fillna(0.0))
            X1 = X1.loc[:, X1.std(ddof=0) > 1e-12]
            ecols = list(X1.columns)
            order = (1, choose_d_by_adf(y1), 0)
            try:
                res = (ARIMA(y1, exog=X1, order=order).fit() if ecols
                       else ARIMA(y1, order=order).fit())
                self._arimax[b] = res
                self._trains[b] = y1.values
                self._exog_tr[b] = X1
                self._ecols_tr[b] = ecols
            except Exception:
                self._arimax[b], self._trains[b] = None, y.values
    ...
    def forecast_x(
        ...
        """ARIMAX forecast with AR(1)-projected iv_pc1."""
        if self._iv_ar1_model is not None:
            try:
                iv_fut_vals = np.asarray(self._iv_ar1_model.forecast(steps))
            except Exception:
                iv_fut_vals = np.full(steps, self._iv_last)
        else:
            iv_fut_vals = np.full(steps, self._iv_last)
        ...
        for b in BETA_COLS:
            m = self._arimax.get(b)
            tr = self._trains.get(b, np.array([]))
            lv = float(tr[-1]) if len(tr) else 0.0
            if m is None:
                fc_b[b] = lv
                continue
            try:
                ecols = self._ecols_tr.get(b, [])
                if not ecols:
                    fc = np.asarray(m.forecast(steps))
                else:
                    Xf = df_macro_pca_fut.reindex(fi).copy()
                    if "iv_pc1" in ecols:
                        Xf["iv_pc1"] = iv_fut_s.values
                    Xf = (Xf.reindex(columns=ecols)
                          .ffill().bfill().fillna(0.0))
                    fc = np.asarray(m.forecast(steps, exog=Xf))
                fc_b[b] = clip_forecast(pd.Series(fc, index=fi),
                                        pd.Series(tr)).values
            except Exception:
                fc_b[b] = lv
...
_record("ARIMAX_NS_macro_pca", yc_ax_m2_bt, is_m2=True)
...
# ── SV_ARIMAX_macro_pca  (IV-augmented SV — M2 pair for SV_ARIMA) ─────────────
print("\n  [SV-X] SV_ARIMAX_macro_pca two-phase fit …")
# Phase 1: train-only λ* + ARIMAX orders
sv_arimax_p1 = SvenssonARIMAXForecaster(
    yc_cols, MATURITY_MAP, SV_LAM1_GRID, SV_LAM2_GRID
)
sv_arimax_p1.fit_x(
    df_yc_train=df_yc.loc[train_idx],
    df_yc_val=df_yc.loc[val_idx],
    df_macro_pca_train=df_macro_tr_pca,
    df_iv_train=df_iv_tr_r,
    verbose=True,
)
# Phase 2: refit on trainval with same λ* and orders
sv_arimax_bt = SvenssonARIMAXForecaster(
    yc_cols, MATURITY_MAP, SV_LAM1_GRID, SV_LAM2_GRID,
    best_orders=sv_arimax_p1.best_orders,
)
sv_arimax_bt.lam1_opt = sv_arimax_p1.lam1_opt
sv_arimax_bt.lam2_opt = sv_arimax_p1.lam2_opt
sv_arimax_bt.refit_arimax_only(
    df_yc_new=df_yc.loc[trainval_idx],
    df_macro_pca=m_tv_pca,
    df_iv=iv_tv,
)
yc_sv_arimax_bt = sv_arimax_bt.forecast_x(TEST_SIZE, test_idx, m_test_pca)
_record("SV_ARIMAX_macro_pca", yc_sv_arimax_bt, is_m2=True)

# ── NS_AR1  (notebook Nelson-Siegel AR(1) model — M1) ────────────────────────
print("\n  [NS-AR1] Nelson-Siegel AR(1) two-phase fit …")
# Phase 1: λ* via expanding-window multi-horizon weighted CV on train only
ns_ar1_p1 = NelsonSiegelAR1Forecaster(
    yc_cols, MATURITY_MAP, NS_LAM_GRID, NS_INITIAL_TRAIN, cv_horizon=TEST_SIZE
)
ns_ar1_p1.fit(df_yc.loc[train_idx], verbose=True)
# Phase 2: refit OLS betas on trainval with same λ*
ns_ar1_bt = NelsonSiegelAR1Forecaster(
    yc_cols, MATURITY_MAP, NS_LAM_GRID, NS_INITIAL_TRAIN, cv_horizon=TEST_SIZE
)
ns_ar1_bt.lam_opt = ns_ar1_p1.lam_opt
ns_ar1_bt.refit(df_yc.loc[trainval_idx])
yc_ns_ar1_bt = ns_ar1_bt.forecast(TEST_SIZE, test_idx)
_record("NS_AR1", yc_ns_ar1_bt, is_m2=False)

# ── NS_AR1_iv  (notebook NS + ARIMAX macro_pca + iv_pc1 — M2 pair) ───────────
print("\n  [NS-AR1-X] NS_AR1_iv two-phase fit …")
ns_ar1x_p1 = NelsonSiegelAR1XForecaster(
    yc_cols=yc_cols, maturity_map=MATURITY_MAP,
    lam_grid=NS_LAM_GRID, initial_train=NS_INITIAL_TRAIN, cv_horizon=TEST_SIZE,
)
ns_ar1x_p1.fit_x(
    df_yc_train=df_yc.loc[train_idx],
    df_macro_pca_train=df_macro_tr_pca,
...
ns_ar1x_bt = NelsonSiegelAR1XForecaster(
    yc_cols=yc_cols, maturity_map=MATURITY_MAP,
    lam_grid=NS_LAM_GRID, initial_train=NS_INITIAL_TRAIN, cv_horizon=TEST_SIZE,
)
ns_ar1x_bt.lam_opt = ns_ar1x_p1.lam_opt
ns_ar1x_bt.refit_x(
    df_yc_new=df_yc.loc[trainval_idx],
    df_macro_pca=m_tv_pca,
    df_iv=iv_tv,
)
yc_ns_ar1x_bt = ns_ar1x_bt.forecast_x(TEST_SIZE, test_idx, m_test_pca)
_record("NS_AR1_iv", yc_ns_ar1x_bt, is_m2=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# §18  BEST PAIR SELECTION  (pair = no-IV model + IV model of same spec)
# ══════════════════════════════════════════════════════════════════════════════

# The 8 model pairs: (tag_without_IV, tag_with_IV)
MODEL_PAIRS: List[Tuple[str, str]] = [
    ("VAR_SV_only", "VAR_SV_only_iv1"),
    ("VAR_SV_macro_pca", "VAR_SV_macro_pca_iv1"),
    ("VAR_NS_only", "VAR_NS_only_iv1"),
    ("VAR_NS_macro_pca", "VAR_NS_macro_pca_iv1"),
    ("SV_ARIMA", "SV_ARIMAX_macro_pca"),
    ("ARIMA_NS", "ARIMAX_NS_macro_pca"),
    ("NS_AR1", "NS_AR1_iv"),
]

# Collect all results into a single dict for lookup
_all_results: Dict[str, float] = {**results_m1, **results_m2}

print("\n" + "─" * 70)
print("§18  Pair-based model selection:")
print(f"  {'Pair (no-IV  /  with-IV)':<50}  {'RMSE_no-IV':>10}  {'RMSE_iv':>10}  {'Total':>10}")
print("  " + "─" * 84)

pair_scores: List[Tuple[float, str, str]] = []  # (total_rmse, tag_no_iv, tag_iv)

for tag_base, tag_iv in MODEL_PAIRS:
    r_base = _all_results.get(tag_base, np.nan)
    r_iv = _all_results.get(tag_iv, np.nan)
    total = (r_base if not np.isnan(r_base) else np.inf) + \
            (r_iv if not np.isnan(r_iv) else np.inf)
    pair_scores.append((total, tag_base, tag_iv))
    base_str = f"{r_base:.5f}" if not np.isnan(r_base) else "   N/A "
    iv_str = f"{r_iv:.5f}" if not np.isnan(r_iv) else "   N/A "
    tot_str = f"{total:.5f}" if total < np.inf else "   N/A "
    print(f"  {tag_base:<26} / {tag_iv:<26}  {base_str:>10}  {iv_str:>10}  {tot_str:>10}")

# Handle forced overrides (env vars still respected)
if CODE4_FORCE_M1_TAG or CODE4_FORCE_M2_TAG:
    best_m1_tag = CODE4_FORCE_M1_TAG if CODE4_FORCE_M1_TAG else min(results_m1, key=results_m1.get)
    best_m2_tag = CODE4_FORCE_M2_TAG if CODE4_FORCE_M2_TAG else min(results_m2, key=results_m2.get)
    rmse_m1_bt = _all_results.get(best_m1_tag, np.nan)
    rmse_m2_bt = _all_results.get(best_m2_tag, np.nan)
    print(f"\n  [FORCED] M1={best_m1_tag}  M2={best_m2_tag}")
else:
    # Select the pair with smallest total_rmse (ignoring pairs with any NaN component)
    valid_pairs = [(tot, tb, ti) for tot, tb, ti in pair_scores if tot < np.inf]
    if valid_pairs:
        best_total, best_m1_tag, best_m2_tag = min(valid_pairs, key=lambda x: x[0])
    else:
        # All pairs have NaN — fall back to independent best
        best_m1_tag = min(results_m1, key=results_m1.get) if results_m1 else "RW"
        best_m2_tag = min(results_m2, key=results_m2.get) if results_m2 else best_m1_tag
        print("  [WARN] No fully-valid pair found — falling back to independent best models")
...
rmse_tot_bt = (
        0.5 * (rmse_m1_bt if not np.isnan(rmse_m1_bt) else rmse_rw)
        + 0.5 * (rmse_m2_bt if not np.isnan(rmse_m2_bt) else rmse_rw)
)

print("\n" + "─" * 70)
print(f"  Best pair: {best_m1_tag}  /  {best_m2_tag}")
print(f"  Best M1 (no-IV): {best_m1_tag:<32}  RMSE={rmse_m1_bt:.5f}  {_vs_rw(rmse_m1_bt)}")
print(f"  Best M2 (w/ IV): {best_m2_tag:<32}  RMSE={rmse_m2_bt:.5f}  {_vs_rw(rmse_m2_bt)}")
...
def _final_yc(tag: str) -> pd.DataFrame:
    ...
    if tag == "NS_AR1":
        # Notebook Nelson-Siegel AR(1): reuse λ* from the back-test phase
        ns_f = NelsonSiegelAR1Forecaster(
            yc_cols, MATURITY_MAP, NS_LAM_GRID, NS_INITIAL_TRAIN, cv_horizon=NS_CV_HORIZON
        )
        ns_f.lam_opt = ns_ar1_p1.lam_opt  # reuse already-found λ*
        ns_f.refit(df_yc.loc[common_idx])
        return ns_f.forecast(6, TRUE_FUTURE_INDEX)

    if tag == "NS_AR1_iv":
        # IV-augmented NS AR(1): reuse λ* from back-test phase
        ns_fx = NelsonSiegelAR1XForecaster(
            yc_cols=yc_cols, maturity_map=MATURITY_MAP,
            lam_grid=NS_LAM_GRID, initial_train=NS_INITIAL_TRAIN, cv_horizon=NS_CV_HORIZON,
        )
        ns_fx.lam_opt = ns_ar1x_p1.lam_opt
        ns_fx.refit_x(
            df_yc_new=df_yc.loc[common_idx],
            df_macro_pca=df_macro_all_pca_f,
            df_iv=df_iv_all_rf,
        )
        return ns_fx.forecast_x(6, TRUE_FUTURE_INDEX, m_fut_pca)

    if tag == "SV_ARIMAX_macro_pca":
        # Two-phase: trainval→orders then refit on all data
        sv_fx_p1 = SvenssonARIMAXForecaster(
            yc_cols, MATURITY_MAP, SV_LAM1_GRID, SV_LAM2_GRID
        )
        sv_fx_p1.fit_x(
            df_yc_train=df_yc.loc[trainval_idx],
            df_yc_val=None,
            df_macro_pca_train=macro_reducer_f.transform(df_macro.loc[trainval_idx]),
            df_iv_train=apply_pca_transform(
                df_iv_feat.loc[trainval_idx], iv_pca_f, iv_sc_f, iv_pca_cols_f
            ),
            verbose=False,
        )
        sv_fx_p1.refit_arimax_only(
            df_yc_new=df_yc.loc[common_idx],
            df_macro_pca=df_macro_all_pca_f,
            df_iv=df_iv_all_rf,
        )
        return sv_fx_p1.forecast_x(6, TRUE_FUTURE_INDEX, m_fut_pca)
...
if not CODE4_SKIP_PLOTS:
    ...
    for ax, col in zip(axes.flat, yc_cols):
        ...
        ax.set_xticks(t);
        ax.set_xticklabels(tl, rotation=45, fontsize=7)
        ax.grid(True, alpha=0.2)
        if col == "ON":
...
for t, r in sorted(results_m2.items(), key=lambda x: x[1]):
    ...
    print(f"    {t:<34} = {r:.5f}  {_vs_rw(r)}{star}")
print()
print("  Pair scores (total_rmse = RMSE_no-IV + RMSE_iv):")
_all_res_summary = {**results_m1, **results_m2}
for tag_base, tag_iv in MODEL_PAIRS:
    r_b = _all_res_summary.get(tag_base, np.nan)
    r_i = _all_res_summary.get(tag_iv, np.nan)
    tot = (r_b if not np.isnan(r_b) else np.inf) + (r_i if not np.isnan(r_i) else np.inf)
    sel = " ◀ SELECTED PAIR" if (tag_base == best_m1_tag and tag_iv == best_m2_tag) else ""
    b_s = f"{r_b:.5f}" if not np.isnan(r_b) else "  N/A  "
    i_s = f"{r_i:.5f}" if not np.isnan(r_i) else "  N/A  "
    t_s = f"{tot:.5f}" if tot < np.inf else "  N/A  "
    print(f"    {tag_base:<26} / {tag_iv:<26}  {b_s} + {i_s} = {t_s}{sel}")
print()
print(f"  Selected M1 (no-IV) : {best_m1_tag}  RMSE={rmse_m1_bt:.5f}")
print(f"  Selected M2 (w/ IV) : {best_m2_tag}  RMSE={rmse_m2_bt:.5f}")
                ...
                "model_pairs": [
                    {
                        "tag_no_iv": tb,
                        "tag_iv": ti,
                        "rmse_no_iv": _safe_float(_all_results.get(tb, np.nan)),
                        "rmse_iv": _safe_float(_all_results.get(ti, np.nan)),
                        "total_rmse": _safe_float(
                            (_all_results.get(tb, np.nan) or 0)
                            + (_all_results.get(ti, np.nan) or 0)
                        ),
                    }
                    for tb, ti in MODEL_PAIRS
                ],
            },
            ensure_ascii=False,
        )
